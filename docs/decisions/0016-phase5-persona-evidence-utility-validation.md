...
4. **Abstention quality.** Cosmetic/grammar-only transformations should
   ...
   `docs/history/experiments/manual-mvp-validation.md`'s Phase 5A section).
...
**Phase 5A's semantic-delta-extraction hypothesis is superseded by Phase
5A Trial 4 / Test 1, which is CLOSED — SUCCESS.** See
`training/phase5a/benchmark/test1-final-result.md` for the authoritative
current result. Trial 3's frozen zero-shot baseline for `Qwen3-0.6B`
(recorded in this decision's history) was the direct predecessor Trial 4
tested specialization against; Trial 4's own decisions and outcome are
recorded in `docs/decisions/0017`.

The full validation-status snapshot and the real Trial 1/2/3/3-addendum/
4-planning results (extraction runs, human-graded groundedness/information-
gain scores, the local-MLX transport change, and Trial 4's original
planning section) are preserved verbatim in
`docs/history/experiments/0016-phase5a-trials-history.md`.
