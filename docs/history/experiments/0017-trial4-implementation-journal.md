# 0017 history — Trial 4 implementation walkthrough + addenda journal

**Historical record, moved out of the main ADR reading path.** Full current
decision context: `docs/decisions/0017-phase5a-trial4-human-filtered-specialization.md`
(Status/Why/the twelve operator decisions — the durable methodology
principles). This file preserves, verbatim, the original infrastructure-only
implementation walkthrough (architecture diagram, the since-superseded
DeepSeek-direct-API `DeepSeekSemanticRevisionJudge` provider and the
since-superseded Correct/Partial/Wrong + Best-response grading model, the
original Python pipeline description, and test counts at that point in time)
plus every addendum written as Trial 4 actually ran: task-contract v2
refinement, generation-reliability/concurrency/Turkish-review support,
structured review decisions + the HDNA Dashboard, the Test 1 v3 two-axis
redesign, and the Test 1 evaluation-stage upgrade (frozen ground truth,
OpenRouter-only DeepSeek, objective metrics).

**Current, non-historical sources of truth:** the canonical contract is
`training/phase5a/lore/task-contract.v3.md` (+ `policy-spec.v1.json`); the
closure result is `training/phase5a/benchmark/test1-final-result.md`; the
current runtime implementation is `extension/src/persona/trial4-*`,
`extension/src/ui/Trial4BenchmarkPanel.svelte`, and
